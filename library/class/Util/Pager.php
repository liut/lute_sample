...
class Util_Pager
{
	...
	function renderLinks($is_return = FALSE)
	{
		...
		if($this->curr > $this->first + 1) {
			empty($this->_labels['first']) || $out .= "<a href=\"". $this->getUrl($this->firstNumber) ."\">" . $this->_labels['first'] . "</a>" . PHP_EOL;
		}
		if($this->curr > $this->first) {
			empty($this->_labels['prev']) || $out .= "<a href=\"". $this->getUrl($this->prevNumber) ."\">" . $this->_labels['prev'] . "</a>" . PHP_EOL;
		}
		foreach($data as $p)
		{
            if ($p['idx'] == $this->curr) {
		...
		if($this->curr < $this->last) {
			empty($this->_labels['next']) || $out .= "<a href=\"". $this->getUrl($this->nextNumber) ."\">" . $this->_labels['next'] . "</a>" . PHP_EOL;
			if ($this->next < $this->last)
			empty($this->_labels['last']) || $out .= "<a href=\"". $this->getUrl($this->lastNumber) ."\">" . $this->_labels['last'] . "</a>" . PHP_EOL;
		}
/*
        for ($i = 0; $i < $this->pageCount; $i++) {
